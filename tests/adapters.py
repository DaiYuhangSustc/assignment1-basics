...
def run_transformer_lm(
    ...
    for i in range(num_layers):
        prefix = f"layers.{i}"
        model.layers[i].ln1.weight.data = weights[f"{prefix}.ln1.weight"]
        model.layers[i].attn.q_proj.weight.data = weights[f"{prefix}.attn.q_proj.weight"]
        model.layers[i].attn.k_proj.weight.data = weights[f"{prefix}.attn.k_proj.weight"]
        model.layers[i].attn.v_proj.weight.data = weights[f"{prefix}.attn.v_proj.weight"]
        model.layers[i].attn.output_proj.weight.data = weights[f"{prefix}.attn.output_proj.weight"]
        model.layers[i].ln2.weight.data = weights[f"{prefix}.ln2.weight"]
        model.layers[i].ffn.w1.weight.data = weights[f"{prefix}.ffn.w1.weight"]
        model.layers[i].ffn.w2.weight.data = weights[f"{prefix}.ffn.w2.weight"]
        model.layers[i].ffn.w3.weight.data = weights[f"{prefix}.ffn.w3.weight"]
